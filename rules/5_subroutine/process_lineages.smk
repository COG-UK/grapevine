...
rule all:
    input:
        expand(config["output_path"] + "/5/{lineage}/traits.csv", lineage=LINEAGES),
        expand(config["output_path"] + "/5/{lineage}/cut_out_trees_done", lineage=LINEAGES)

rule iq_tree:
    input:
        lineage_fasta = config["output_path"] + "/5/lineage_{lineage}.fasta"
    params:
        lineage = "{lineage}",
        outgroup = lambda wildcards: lineage_to_outgroup_map[wildcards.lineage]
    output:
        tree = config["output_path"] + "/5/{lineage}/cog_gisaid_{lineage}.tree"
    log:
        config["output_path"] + "/logs/5_iq_tree_{lineage}.log"
...
rule phylotype_tree:
    ...
    output:
        tree = config["output_path"] + "/5/{lineage}/cog_gisaid_{lineage}.phylotyped.tree"
    params:
        lineage="{lineage}",
        collapse=5E-6,
        threshold=2E-5,
    log:
        config["output_path"] + "/logs/5_phylotype_{lineage}.log"
    shell:
        """
...
rule annotate_tree:
    ...
    params:
        lineage = "{lineage}",
    output:
        tree = config["output_path"] + "/5/{lineage}/cog_gisaid_{lineage}.phylotyped.annotated.tree"
    log:
...
rule acctran_ancestral_reconstruction:
    ...
    params:
        lineage = "{lineage}",
    output:
        tree = config["output_path"] + "/5/{lineage}/cog_gisaid_{lineage}.phylotyped.annotated.acc.tree"
    log:
        config["output_path"] + "/logs/5_acctran_ancestral_reconstruction_{lineage}.log"
    shell:
        """
...
rule deltran_ancestral_reconstruction:
    ...
    params:
        lineage = "{lineage}",
    output:
        tree = config["output_path"] + "/5/{lineage}/cog_gisaid_{lineage}.phylotyped.annotated.acc.del.tree"
    log:
        config["output_path"] + "/logs/5_deltran_ancestral_reconstruction_{lineage}.log"
    shell:
        """
        clusterfunk ancestral_reconstruction \
...
rule push_lineage_to_tips:
    ...
    params:
        lineage = "{lineage}",
    output:
        tree = config["output_path"] + "/5/{lineage}/cog_gisaid_{lineage}.phylotyped.annotated.acc.del.uk_lineages.tree"
    log:
        config["output_path"] + "/logs/5_push_lineage_to_tips_{lineage}.log"
    shell:
        """
...
rule label_acctran_introductions:
    ...
    params:
        lineage = "{lineage}",
    output:
        tree = config["output_path"] + "/5/{lineage}/cog_gisaid_{lineage}.phylotyped.annotated.acc.del.uk_lineages.acc_labelled.tree"
    log:
        config["output_path"] + "/logs/5_label_acctran_introductions_{lineage}.log"
    shell:
        """
...
rule label_deltran_introductions:
    ...
    output:
        tree = config["output_path"] + "/5/{lineage}/cog_gisaid_{lineage}.phylotyped.annotated.acc.del.uk_lineages.acc_labelled.del_labelled.tree"
    log:
        config["output_path"] + "/logs/5_label_deltran_introductions_{lineage}.log"
    shell:
        """
        clusterfunk label_transitions \
...
rule output_annotations:
    ...
    params:
        lineage = "{lineage}",
    output:
        traits = config["output_path"] + "/5/{lineage}/traits.csv"
    log:
        config["output_path"] + "/logs/5_output_annotations_{lineage}.log"
    shell:
        """
