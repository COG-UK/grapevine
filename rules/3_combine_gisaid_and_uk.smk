        ...
        with open(str(output.fasta), "w") as fasta, open(str(output.metadata), "w") as metadata:
            ...
            for key, value in hash_dict.items():
                if len(value) == 1:
                    ...
                    for redundant in value[1:]:
                        metadata.write(r.id + ",")
                        metadata.write(redundant + "\n")
...
rule gisaid_output_full_lineage_table:
    input:
        fasta = config["GISAID_background_expanded_fasta"],
        metadata = config["GISAID_background_expanded_metadata"],
    output:
        fasta = config["output_path"] + "/3/gisaid.matched.full.fasta",
        metadata = config["output_path"] + "/3/gisaid.matched.lineages.full.csv",
    log:
...
rule combine_gisaid_and_cog_expanded:
    input:
        previous_stage = config["output_path"] + "/logs/2_summarize_pangolin_lineage_typing.log",
        gisaid_fasta = rules.gisaid_output_full_lineage_table.output.fasta,
        gisaid_metadata = rules.gisaid_output_full_lineage_table.output.metadata,
        uk_fasta = rules.uk_output_full_lineage_table.output.fasta,
        uk_metadata = rules.uk_output_full_lineage_table.output.metadata
    output:
